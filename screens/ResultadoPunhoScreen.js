...
// Tamanho dos ROIs para Punho/Antebraço (formato quadrado)
const FOREARM_ROI_SIZES = {
  'UD':    { width: 60, height: 60 },
  'MID':   { width: 60, height: 60 },
  '1/3':   { width: 60, height: 60 },
  'Total': { width: 70, height: 70 },
};
...
const ForearmROI = ({ 
  ...
  const roiWidth = customSize?.width || (FOREARM_ROI_SIZES[region.id] || { width: 100 }).width;
  const roiHeight = customSize?.height || (FOREARM_ROI_SIZES[region.id] || { height: 50 }).height;
...
export default function ResultadoPunhoScreen({ route }) {
  const { 
    id, 
    nome, 
    idade,
  ...
  const regioesComPosicoes = regioesPunho.map(r => {
    ...
    <ScrollView scrollEnabled={scrollEnabled} style={[styles.container, { backgroundColor: theme.editorBg }]} contentContainerStyle={[styles.scrollContent, (isNarrow || isMobile) && { paddingBottom: 70 }]}>
      ...
      >
        ...
        <View style={styles.headerTitleContainer}>
          ...
          <Text style={[styles.title, { color: theme.text }]}>Punho</Text>
        </View>
      ...
      <Animated.View style={[styles.patientBar, { backgroundColor: theme.editorSurface, borderColor: theme.editorBorder, opacity: fadeAnim }]}>
        <View style={styles.patientInfo}>
          <FontAwesome5 name="user" size={12} color="#9B59B6" />
        ...
        <View style={styles.canvasContainer}>
            ...
            >
              ...
              >
                {imagemDoExame ? (
                  ...
                  <View style={styles.noImageContainer}>
                    <FontAwesome5 name="hand-paper" size={48} color="#4A5568" />
                    <Text style={styles.noImageText}>Imagem do Antebraço</Text>
                ...
                {regioesComPosicoes.map((r) => (
                  <ForearmROI
                    ...
                    customSize={customRoiSizes[r.id] || { width: (FOREARM_ROI_SIZES[r.id] || { width: 100 }).width * roiScale, height: (FOREARM_ROI_SIZES[r.id] || { height: 50 }).height * roiScale }}
                  />
                ))}
              </View>
              ...
              {activeTool === 'pan' && (
                <View style={styles.toolIndicator} pointerEvents="none">
                  <FontAwesome5 name="hand-paper" size={16} color="rgba(155, 89, 182, 0.5)" />
                </View>
              )}
            </View>
          </View>
          ...
          <View style={styles.canvasInfoBar}>
            <Text style={styles.canvasInfoText}>
              <FontAwesome5 name="crosshairs" size={10} color="#9B59B6" /> ROI: {selectedId || 'Nenhuma'}
            </Text>
            <Text style={styles.canvasInfoText}>
              <FontAwesome5 name="search" size={10} color="#9B59B6" /> Zoom: {Math.round(zoomLevel * 100)}%
            </Text>
            <Text style={styles.canvasInfoText}>
              <FontAwesome5 name="sun" size={10} color="#9B59B6" /> Brilho: {brightness}%
            </Text>
            <Text style={styles.canvasInfoText}>
          ...
          {showAdjustments && (
            <View style={[styles.adjustmentPanel, { backgroundColor: theme.editorPanel }]}>
              <Text style={[styles.panelTitle, { color: theme.editorText }]}>
                <FontAwesome5 name="sliders-h" size={12} color="#9B59B6" /> Ajustes de Imagem
              </Text>
              ...
              <View style={styles.sliderContainer}>
                <View style={styles.sliderHeader}>
                  <FontAwesome5 name="adjust" size={12} color="#9B59B6" />
            ...
            <View style={[styles.roiPanel, { backgroundColor: theme.editorPanel }]}>
              ...
              {regiaoSelecionada ? (
                <View style={styles.roiData}>
                  ...
                  {/* Informações adicionais de área e BMC */}
                  ...
                  {/* Nota sobre região 1/3 */}
                  {regiaoSelecionada.id === '1/3' && (
                    <View style={styles.notePanel}>
                      <FontAwesome5 name="info-circle" size={10} color="#3498DB" />
                      <Text style={styles.noteText}>
                        A região 1/3 (33%) é recomendada para diagnóstico quando coluna ou quadril não podem ser avaliados.
                      </Text>
                    </View>
                  )}
                </View>
                ...
                <View style={styles.roiEmpty}>
                  <FontAwesome5 name="hand-pointer" size={24} color="#9B59B6" />
                  <Text style={[styles.roiEmptyText, { color: theme.textMuted }]}>Selecione uma região do antebraço</Text>
                </View>
              )}
            </View>
          )}
      ...
      {showSaveAnimation && (
        <View style={styles.saveOverlay}>
          ...
          <Animated.Text style={[styles.saveText, { opacity: saveSuccessAnim }]}>Exame do Punho Salvo!</Animated.Text>
        </View>
      )}
    </ScrollView>
...
const createStyles = (theme) => StyleSheet.create({
  container: { flex: 1, backgroundColor: theme.editorBg },
  scrollContent: { paddingBottom: 40 },
  fixedToolbar: {
    flexDirection: 'row',
    backgroundColor: theme.editorSurface,
    borderTopWidth: 1,
    borderTopColor: theme.editorBorder,
    paddingVertical: 6,
    paddingHorizontal: 8,
  ...
  fixedToolBtn: {
    ...
    alignItems: 'center',
    backgroundColor: theme.editorPanel,
    marginHorizontal: 2,
  },
  fixedToolBtnActive: {
  ...
  header: {
    ...
    backgroundColor: theme.editorSurface,
    borderBottomWidth: 1,
    borderBottomColor: theme.editorBorder,
  },
  backButton: {
    width: 36,
    height: 36,
    ...
    alignItems: 'center',
    borderWidth: 1,
    borderColor: theme.editorBorder,
  },
  headerTitleContainer: { flex: 1, alignItems: 'center' },
  headerSubtitle: { fontSize: 10, fontWeight: '600', color: '#9B59B6', letterSpacing: 2 },
  title: { fontSize: 16, fontWeight: '700', color: theme.text, marginTop: 2 },
  headerRight: { width: 80, alignItems: 'flex-end' },
  statusIndicator: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  statusDot: { width: 6, height: 6, borderRadius: 3 },
  statusText: { fontSize: 10, color: theme.textMuted },
  patientBar: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
  ...
  patientText: { fontSize: 12, color: theme.editorText, fontWeight: '500' },
  patientDivider: { width: 1, height: 14, backgroundColor: theme.editorBorder },
  editorContainer: {
    flexDirection: 'row',
    marginHorizontal: 12,
    marginTop: 12,
  ...
  toolbar: {
    ...
    alignItems: 'center',
    borderRightWidth: 1,
    borderRightColor: theme.editorBorder,
  },
  toolButton: { width: 36, height: 36, borderRadius: 8, justifyContent: 'center', alignItems: 'center', marginVertical: 3 },
  toolButtonActive: { backgroundColor: '#9B59B6' },
  zoomIndicator: { marginTop: 'auto', paddingVertical: 8, paddingHorizontal: 4, backgroundColor: theme.editorBorder, borderRadius: 6 },
  zoomText: { fontSize: 9, color: theme.textMuted, fontWeight: '600' },
  canvasContainer: { flex: 1, backgroundColor: theme.editorBg },
  rulerTop: {
    height: 20,
    backgroundColor: theme.editorPanel,
    flexDirection: 'row',
  ...
  rulerMark: { flex: 1, alignItems: 'center', borderLeftWidth: 1, borderLeftColor: theme.editorBorder, paddingBottom: 2 },
  rulerText: { fontSize: 8, color: theme.textFaint },
  imageArea: { flex: 1, flexDirection: 'row' },
  rulerLeft: { width: 20, backgroundColor: theme.editorPanel, borderRightWidth: 1, borderRightColor: theme.editorBorder },
  rulerMarkVertical: { flex: 1, justifyContent: 'center', alignItems: 'center', borderTopWidth: 1, borderTopColor: theme.editorBorder },
  imageCanvas: { 
    flex: 1, 
  ...
  canvasInfoText: { fontSize: 10, color: theme.textMuted },
  sidePanel: { width: 220, backgroundColor: theme.editorPanel, borderLeftWidth: 1, borderLeftColor: theme.editorBorder },
  adjustmentPanel: { padding: 12, borderBottomWidth: 1, borderBottomColor: theme.editorBorder },
  panelTitle: { fontSize: 11, fontWeight: '700', color: theme.editorText, marginBottom: 12, textTransform: 'uppercase', letterSpacing: 0.5 },
  sliderContainer: { marginBottom: 16 },
  sliderHeader: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 },
  sliderLabel: { fontSize: 11, color: theme.textMuted, flex: 1 },
  sliderValue: { fontSize: 11, color: '#9B59B6', fontWeight: '600' },
  slider: { width: '100%', height: 24 },
  resetButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: theme.editorBorder, paddingVertical: 8, borderRadius: 6 },
  resetButtonText: { fontSize: 11, color: theme.editorText, fontWeight: '600' },
  roiPanel: { padding: 12, borderBottomWidth: 1, borderBottomColor: theme.editorBorder },
  roiData: { gap: 12 },
  roiHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' },
  roiRegionName: { fontSize: 18, fontWeight: '700', color: theme.text },
  roiRegionDesc: { fontSize: 10, color: theme.textMuted, marginTop: 2 },
  ...
  roiMetricItem: { flex: 1, backgroundColor: theme.editorBg, padding: 8, borderRadius: 6, alignItems: 'center', borderWidth: 1, borderColor: theme.editorBorder },
  roiMetricLabel: { fontSize: 9, color: theme.textMuted, marginBottom: 4 },
  roiMetricValue: { fontSize: 14, fontWeight: '700', color: theme.text },
  roiMetricUnit: { fontSize: 8, color: theme.textFaint, marginTop: 2 },
  additionalMetrics: { backgroundColor: theme.editorBg, padding: 10, borderRadius: 6, borderWidth: 1, borderColor: theme.editorBorder },
  additionalMetricRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 4 },
  additionalMetricLabel: { fontSize: 10, color: theme.textMuted },
  additionalMetricValue: { fontSize: 10, color: theme.text, fontWeight: '600' },
  tScoreBar: { marginTop: 8 },
  tScoreBarBg: { height: 8, flexDirection: 'row', borderRadius: 4, overflow: 'hidden' },
  tScoreZone: { height: '100%' },
  tScoreMarker: { position: 'absolute', top: -2, width: 4, height: 12, backgroundColor: theme.text, borderRadius: 2, marginLeft: -2 },
  tScoreLabels: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 },
  tScoreLabel: { fontSize: 8, color: theme.textFaint },
  notePanel: { flexDirection: 'row', backgroundColor: theme.editorBg, padding: 10, borderRadius: 6, borderWidth: 1, borderColor: '#3498DB33', gap: 8, alignItems: 'flex-start' },
  noteText: { flex: 1, fontSize: 9, color: theme.textMuted, lineHeight: 14 },
  roiEmpty: { alignItems: 'center', paddingVertical: 20, gap: 8 },
  roiEmptyText: { fontSize: 11, color: theme.textMuted, textAlign: 'center' },
  ...
  roiSizeBarContainer: {
    flex: 1,
    height: 6,
    backgroundColor: theme.editorBorder,
    borderRadius: 3,
    overflow: 'hidden',
  },
  roiSizeBar: {
  ...
  regionItemActive: { backgroundColor: 'rgba(155, 89, 182, 0.15)', borderColor: '#9B59B6' },
  regionDot: { width: 8, height: 8, borderRadius: 4, marginRight: 8 },
  regionItemContent: { flex: 1 },
  regionItemText: { fontSize: 12, color: theme.textMuted, fontWeight: '600' },
  regionItemTextActive: { color: theme.text },
  regionItemSubtext: { fontSize: 9, color: theme.textFaint, marginTop: 2 },
  regionItemValue: { fontSize: 11, color: theme.textFaint, fontWeight: '600' },
  controls: { paddingHorizontal: 12, paddingTop: 16, flexDirection: 'row', gap: 12 },
  button: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#9B59B6', borderRadius: 10, paddingVertical: 14, gap: 10 },
